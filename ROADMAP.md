...
These items are adoption blockers or correctness gaps at the distribution layer. None of them affect mutation correctness on supported inputs, but each would cause a Haskell community member to reasonably question whether the package is production-ready.

- [ ] Establish a forward-GHC-compatibility process: the current `build-depends: ghc >= 9.12 && < 9.13` bound will make the package unbuildable when GHC 9.14 ships; add a lightweight nightly or weekly CI job (separate from the PR path, so it does not affect the five-minute budget) that builds and tests against GHC HEAD or the next release candidate; document in `CLAUDE.md` that bumping the upper bound is a required step in the release checklist whenever a new GHC major version ships; the tight bound is deliberate (GHC internal APIs break between releases) but the process for updating it must be explicit

- [ ] Hide internal library modules from the public API: `Test.MuCheck.Utils.Syb`, `Test.MuCheck.Utils.Helpers`, `Test.MuCheck.Utils.Common`, and `Test.MuCheck.Utils.Print` are currently listed under `exposed-modules` in `MuCheck.cabal`, making them part of the public API contract at 1.0; move them to `other-modules` or rename them under a `Test.MuCheck.Internal.*` namespace; a 1.0 library surface should not expose implementation utilities that external callers are not expected to use, because any downstream package that imports them forces a major-version bump whenever the internals change

- [ ] Resolve the Hackage naming conflict before publishing: the name `MuCheck` is already registered on Hackage (by the original Duc Lee / Rahul Gopinath project); uploading this fork under the same name will either be rejected by Hackage or silently shadow the existing package; contact the current Hackage trustees to request a maintainer transfer, or publish under a distinct name (e.g. `mucheck-ghc`) and document the relationship to the original in the cabal description field; do not upload to Hackage under the existing name without resolving this first

- [ ] Document the scope of `hint`-based evaluation in README: mucheck cannot evaluate modules that depend on C FFI bindings, Template Haskell splices that require compiled dependencies, or packages that are not present in the active GHC package database; when evaluation fails for these reasons the current behaviour is an interpreter error counted as `_maErrors`, which looks identical to a real runtime failure; add a "Limitations" section to README that lists these exclusions explicitly and explains the `--write-ghc-environment-files=always` prerequisite; add a diagnostic hint to the error output when a `GhcException` or `NotAllowed` error suggests a missing module

- [ ] Explicitly document Unix-only support: `MuCheck.cabal` lists `unix >= 2.7` as a dependency, making the package silently unbuildable on Windows; add `os(windows)` to a `if` block in the cabal file that either provides a stub or emits a clear build-time error, and state in the README synopsis that Windows is not supported; do not claim or imply cross-platform support in documentation or Hackage metadata

- [ ] Publish expected runtime characteristics: mutation testing is inherently slow and users need a calibration point before committing to it in CI; add a "Performance" section to README documenting the approximate time per mutant on a representative machine (e.g. "~0.5s per mutant with `--workers 4` on a four-core laptop"), how `--workers N` scales, and a recommended CI configuration (`--timeout`, `--max-mutants`, `--git-diff-lines`) for keeping runs under two minutes; without this, the first experience for most users is an unexpectedly long hang with no progress indication beyond the live counter