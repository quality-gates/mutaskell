...
read `CODING_STANDARDS.md`. Its production mutation gate is a hard merge
requirement: production Haskell in `src/` and `app/` needs covered-MSI of at
least 80% for the changed scope.
...
| `src/Test/Mutaskell.hs` | Top-level orchestration: generate → sample → evaluate → summarise |
| `src/Test/Mutaskell/Mutation.hs` | All mutant generation: literal ops, bool ops, if/else, guards, function/operator substitution, pattern match permutation |
| `src/Test/Mutaskell/MuOp.hs` | `MuOp` type and `Mutable` class; AST type aliases |
| `src/Test/Mutaskell/Config.hs` | `Config`, `MuVar`, `FnOp`; default operator groups |
| `src/Test/Mutaskell/Interpreter.hs` | hint-based mutant evaluation; `stopFast` short-circuit logic |
| `src/Test/Mutaskell/TestAdapter.hs` | `Summarizable` / `TRun` type class interface for test frameworks |
| `src/Test/Mutaskell/AnalysisSummary.hs` | `MAnalysisSummary` type and its `Show` instance |
| `src/Test/Mutaskell/Tix.hs` | HPC `.tix` / `.mix` parsing for coverage-guided mutation |

## Smoke test

The `hint` interpreter resolves modules at runtime from the GHC package database. Running the bare binary directly fails because `Test.Mutaskell.TestAdapter.AssertCheck` is not installed system-wide. The correct procedure is:
...
2. **Run the smoke test** — build with `--write-ghc-environment-files=always`, then `cabal run mutaskell -- Examples/AssertCheckTest.hs`. Confirm the kill count has not dropped.
...
6. **Bump the version in the PR** — pick the next semver tag and update the `version:` field and `source-repository this` tag in `mutaskell.cabal` *before opening the PR*. This must land in the same PR as the change, not after. Verify with `git diff $(git describe --tags --abbrev=0)..HEAD -- mutaskell.cabal | grep version` that the version moves forward, not backward. If the bump is deferred until after merge, branches cut from main for follow-up fixes will start from the old version and silently revert it when they land.
7. **Commit and push** — fix forward only. No `--force-push` and no `--amend` on published commits. Do not use `reset --hard` either. Do not ask the user to run destructive git commands for you. If a hook or check fails, fix it in a new commit. **The `main` branch has push protection — all changes must land via a PR.**
8. **Watch CI** — wait for the Actions run to go green before merging. Run `gh pr checks <number>` to confirm every workflow passes; do not merge if any is red. Also check for inline code-scanning comments (HLint posts findings as PR review comments); fix any warnings before merging. Use `gh api repos/quality-gates/mutaskell/pulls/<number>/comments --jq '.[].body'` to list them.
9. **Merge to main** — squash or merge commit, then push main. Ensure local main is in sync with new origin/main.
...
- GHC 9.12.1 (Homebrew) works locally but is not in the CI matrix. Avoid using APIs newer than GHC 9.2; check the `tested-with` field in `mutaskell.cabal`.
...
**Do not assert on hardcoded mutation counts.** Counts change whenever a mutator is added or modified. They are implementation details, not public behaviour. The commented-out tests in `test/Test/Mutaskell/MutationSpec.hs` that assert on raw AST values are left as reference — do not uncomment and rely on them.
