...
| `src/Test/Muskell.hs` | Top-level orchestration: generate → sample → evaluate → summarise |
| `src/Test/Muskell/Mutation.hs` | All mutant generation: literal ops, bool ops, if/else, guards, function/operator substitution, pattern match permutation |
| `src/Test/Muskell/MuOp.hs` | `MuOp` type and `Mutable` class; AST type aliases |
| `src/Test/Muskell/Config.hs` | `Config`, `MuVar`, `FnOp`; default operator groups |
| `src/Test/Muskell/Interpreter.hs` | hint-based mutant evaluation; `stopFast` short-circuit logic |
| `src/Test/Muskell/TestAdapter.hs` | `Summarizable` / `TRun` type class interface for test frameworks |
| `src/Test/Muskell/AnalysisSummary.hs` | `MAnalysisSummary` type and its `Show` instance |
| `src/Test/Muskell/Tix.hs` | HPC `.tix` / `.mix` parsing for coverage-guided mutation |

## Smoke test

The `hint` interpreter resolves modules at runtime from the GHC package database. Running the bare binary directly fails because `Test.Muskell.TestAdapter.AssertCheck` is not installed system-wide. The correct procedure is:

```bash
cabal build --write-ghc-environment-files=always all
cabal run muskell -- Examples/AssertCheckTest.hs
```
...
2. **Run the smoke test** — build with `--write-ghc-environment-files=always`, then `cabal run muskell -- Examples/AssertCheckTest.hs`. Confirm the kill count has not dropped.
...
6. **Bump the version in the PR** — pick the next semver tag and update the `version:` field and `source-repository this` tag in `muskell.cabal` *before opening the PR*. This must land in the same PR as the change, not after. Verify with `git diff $(git describe --tags --abbrev=0)..HEAD -- muskell.cabal | grep version` that the version moves forward, not backward. If the bump is deferred until after merge, branches cut from master for follow-up fixes will start from the old version and silently revert it when they land.
...
- GHC 9.12.1 (Homebrew) works locally but is not in the CI matrix. Avoid using APIs newer than GHC 9.2; check the `tested-with` field in `muskell.cabal`.
...
**Do not assert on hardcoded mutation counts.** Counts change whenever a mutator is added or modified. They are implementation details, not public behaviour. The commented-out tests in `test/Test/Muskell/MutationSpec.hs` that assert on raw AST values are left as reference — do not uncomment and rely on them.
