...
6. **Bump the version in the PR** — pick the next semver tag and update the `version:` field and `source-repository this` tag in `mutaskell.cabal` *before opening the PR*. This must land in the same PR as the change, not after. Verify with `git diff $(git describe --tags --abbrev=0)..HEAD -- mutaskell.cabal | grep version` that the version moves forward, not backward. If the bump is deferred until after merge, branches cut from master for follow-up fixes will start from the old version and silently revert it when they land.
