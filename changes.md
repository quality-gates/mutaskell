...
## [0.8.23]
  ...
  * Added: classified worker errors for malformed or missing workload transport data; per-child transport files are cleaned up after each evaluation. `bench/worker.sh` measures end-to-end worker runs (wall time, peak RSS) across worker counts.
...
## [0.8.22]
  ...
  * Added: a generation phase profile in `bench/mutation.sh` and `bench/mutation-bench.hs` reporting selector work, operator sampling, AST application, mutator/span deduplication, rendering and source deduplication separately over growing inputs (#34).
...
## [0.8.0]
  ...
  * Note: a very large module (mutaskell's own 1454-line `Mutation.hs`) can still yield zero mutants, because every mutant re-renders the whole module via `exactPrint` (~0.5s each) and generating even one exceeds the budget. The real fix is a generator-representation change (incremental/diff rendering); tracked in `docs/orchestrator-roadmap.md`
  * Added: project mode — point mutaskell at a directory (`mutaskell .` or `mutaskell ~/pandoc`) and it drives the project's own build/test across the whole repo, no `--exec` or flags required. It auto-detects the build/test commands (cabal: `cabal build all` / `cabal test all`; stack: `stack build` / `stack test`), discovers source files from the `hs-source-dirs` declared in the project's `.cabal` files plus each package directory, runs the baseline build+test exactly once, then mutates each file in turn and reports a single project-level score
  * Added: project mode is resilient — a file that fails to parse or whose generation blows up is logged and skipped, and the run continues rather than aborting
  * Added: project mode is resumable — completed files are recorded in `.mutaskell/progress`, so re-running after an interrupt skips finished work; surviving mutants are written to `.mutaskell/survivors.txt` with a before/after line diff. All run-state lives under a single `.mutaskell/` directory so the rest of the working tree stays clean (gitignore one entry)
  * Added: `--jobs N` parallel project runs. The discovered files are sharded across N isolated copies of the repo (rsync'd minus `.git`/`dist-newstyle`/`.mutaskell`), one worker subprocess per shard, and the results merged into a single project score. Isolation is required because the orchestrator edits files in place. Verified on the demo: ~1.5x at N=2 with correct result merge. Honest limits: file-level sharding can be uneven on few-file repos, and each worker pays a cold first build (so the speedup shows once per-mutant build+test dominates); the clean "≥2x at 4 jobs" target needs a balanced, many-file, build/test-dominated workload
  * Added: budget flags `--max-mutants N` (cap total mutants across the run) and `--time-budget SECONDS` (stop after a wall-clock budget); either way the run ends early and reports a partial score instead of running unbounded. `--max-mutants` is now a real CLI flag, not config-only
  * Added: coverage-guided generation in project/orchestrator mode. With `--tix FILE` or `--coverage`, operators whose target span lies in code not exercised by the test suite (per the HPC `.tix`) are dropped *before* sampling and rendering, so the sample budget and every build/test cycle is spent only on covered code. Verified on the demo: Calc.hs drops from 12 to 11 mutants once the untested `twice` is gated out, consistently and driven by the tix
  * Fixed: parallel project runs (`--jobs N`) no longer hide worker failures. The master ignored worker exit codes, so a worker whose baseline build/test failed (or that crashed) exited before writing its result and its whole shard vanished silently — the merged score looked clean and green. The master now checks each worker's exit code and, if any failed, prints a loud error naming the failed workers and exits 3 instead of reporting an incomplete score as success
  * Fixed: project mode no longer mutates test-suite/benchmark source code. Discovery is now stanza-aware — `hs-source-dirs` under `library`/`executable` are mutated; those under `test-suite`/`benchmark` are excluded from the walk. Mutating the tests themselves is meaningless for mutation testing and polluted the score (e.g. the demo dropped from 35 to 12 mutants once `test/` was excluded)
  * Fixed: `--jobs` worker copies no longer inherit a stale `.ghc.environment.*` / `cabal.project.local` from the original repo (rsync now excludes them); those bake in absolute paths to the original `dist-newstyle` and could misdirect a worker's toolchain or hint-based tests
  * Fixed: the parallel master no longer crashes on a malformed worker result file (a worker killed mid-write left a partial line that a bare `read` threw on); parsing is now defensive
  * Fixed: coverage gating only applies when the module name resolves to exactly one entry in the `.tix`. In a multi-package repo two modules can share a name; the old code picked the first match and could apply the wrong module's coverage (dropping covered mutants or keeping uncovered ones). 0 or >1 matches now means "do not gate", which is safer than gating with the wrong data
  * Fixed: parallel project runs (`--jobs N`) are now resumable. The master reads `.mutaskell/progress`, shards only the not-yet-done files, and merges each worker's completed-file list back in, so a re-run skips finished work and reports "all done" when nothing remains (previously each worker's progress lived in an ephemeral copy and was discarded)
  * Fixed: a file whose generation exceeds the time budget is no longer silently recorded as "done" with zero mutants (which made resume skip it forever); generation now reports whether it completed, and only genuinely-finished files are recorded done
  * Added: tests for the new surface — CLI round-trips for `--exec`/`--build-cmd`/`--test-cmd`/`--max-mutants`/`--time-budget`/`--jobs` (and bad-int rejection), `adjacentSwaps` (n-1 swaps, empty for singletons), and `genSampledMutants` (non-empty + respects the `maxNumMutants` cap)
  * Fixed: `getUnCoveredPatches` now reads only the requested module's `.mix` instead of requiring every module in the `.tix` to resolve. A multi-module `.tix` (e.g. a cabal project whose test-suite modules' `.mix` files live in a different directory) previously failed the whole coverage lookup if any single `.mix` was missing
  * Changed: function-clause reordering now generates adjacent-pair swaps (`n-1` variants) instead of the full `n!` permutation set. The factorial set made generation blow up super-linearly — and then quadratically through the dedup pass — on functions with many clauses (the dominant cause of the ~23% of real-repo files whose generation previously timed out); adjacent swaps still exercise clause-order sensitivity while keeping generation linear
  ...
  * Added: CPP-aware parsing (`getASTFromFile`) used by `--exec` and `--dry-run`. A sweep of five large repos found every sampled parse failure (20/20) was a CPP file; the string parser does not run the C preprocessor. CPP files now parse via `ghc-exactprint`'s `parseModuleWithCpp`, with cabal's `cabal_macros.h` auto-discovered under `dist-newstyle` for `MIN_VERSION_*` guards. Verified on the Pandoc clone: `Class/CommonState.hs` went from a parse error to 8 generated mutants
  * Fixed: `--dry-run` no longer crashes (`Prelude.maximum: empty list`) on files that generate zero mutants, such as pure re-export modules (`Text.Pandoc`, `Data.Aeson`); it now reports `Total 0`
  * Note: each `--exec` mutant triggers a real recompile + test run, so it is slow on large repos, and generation can still blow up on literal-dense files (guarded by a timeout). See `docs/orchestrator-roadmap.md` for the measured evidence and next steps
