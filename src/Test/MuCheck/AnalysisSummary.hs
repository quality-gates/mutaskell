...
data MAnalysisSummary = MAnalysisSummary
    { _maCoveredNumMutants :: Int
    -- ^ The number of mutants that was covered by test cases
    , _maNumMutants :: Int
    -- ^ The number of mutants tested (after sampling)
    , _maAlive :: Int
...
instance Show MAnalysisSummary where
    show (MAnalysisSummary{..}) =
        let noerrors = mnum - _maErrors
            mnum = max _maCoveredNumMutants _maNumMutants
            showx a = if a == -1 then "not provided" else show a
            msi
                | noerrors > 0 = _maKilled * 100 `div` noerrors
                | otherwise = 0
         in showAS
                [ "Mutation score (MSI): " ++ show msi ++ "%"
